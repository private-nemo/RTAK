...
This note applies to ATAK clients on OmniNodes, not to the relay node itself (the relay has no display or map UI). It is included here for operator reference.
...
2. **Local tile server on the OmniNode** — run TileServer GL or a lightweight mbtiles HTTP server on the Pi 4B. ATAK clients pull tiles from the node's LAN IP. All requests remain on the private local network and never reach the internet.
